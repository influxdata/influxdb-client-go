## 3.0.0 [in progress]
- Complete rewrite. More idiomatic API with better user experience. Improved code readability.
- API is not compatible with v2, but it is on par with v2 features.
- Main differences:
  - Available on import path `github.com/influxdata/influxdb-client-go/v3/influxclient`
  - Package is `inluxclient`
  - Client communicates with single organization, organization name or id is set in `Params`
  - Function `WriteData` of `Client` and `PointsWriter` allows writing any custom annotated type.
  - Single `Query` function returning `QueryResultReader` with the `Decode` function to deserializes values into any custom annotated type or map.  
  - No `Setup` method
  - Gzip for sending data is used automatically when the size of package exceeds the limit according to the `WriteParams.GzipThreshold`.
  - The default HTTPClient is used automatically. No HTTP specific options are available. To change HTTP options set a custom HTTPClient.
- Read [Readme](https://github.com/influxdata/influxdb-client-go/README.md) and study examples to get familiar with new API 
