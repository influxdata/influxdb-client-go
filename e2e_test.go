...
func TestE2E(t *testing.T) {
	...
	influx, err := influxdb.New(nil, influxdb.WithAddress("http://127.0.0.1:8086"), influxdb.WithUserAndPass("e2e-test-user", "e2e-test-password"))
	if err != nil {
		t.Fatal(err)
	}
	...
	r, err := influx.QueryCSV(context.Background(), `from(bucket:bucket)|>range(start:-1000h)|>group()`, `e2e-test-org`, struct{ bucket string }{bucket: "e2e-test-bucket"})
	if err != nil {
		t.Fatal(err)
		t.Fatal(err)
	}
