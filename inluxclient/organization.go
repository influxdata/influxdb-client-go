...
type OrganizationAPI struct {
	client *model.ClientWithResponses
}

// newOrganizationAPI returns new OrganizationAPI instance
func newOrganizationAPI(client *model.ClientWithResponses) *OrganizationAPI {
...
func (o *OrganizationAPI) getOrganizations(ctx context.Context, filter *Filter) ([]model.Organization, error) {
	...
	response, err := o.client.GetOrgsWithResponse(ctx, params)
	if err != nil {
		return nil, fmt.Errorf("cannot retrieve organizations: %w", err)
	}
	if response.JSONDefault != nil {
		return nil, fmt.Errorf("cannot retrieve organizations: %s: %s", response.JSONDefault.Code, response.JSONDefault.Message)
	}
...
func (o *OrganizationAPI) Create(ctx context.Context, org *model.Organization) (*model.Organization, error) {
	params := &model.PostOrgsParams{}
	req := model.PostOrgsJSONRequestBody{
		Name:        org.Name,
		Description: org.Description,
	}
	response, err := o.client.PostOrgsWithResponse(ctx, params, req)
	if err != nil {
		return nil, fmt.Errorf("cannot create organization: %w", err)
	}
	if response.JSONDefault != nil {
		return nil, fmt.Errorf("cannot create organization: %s: %s", response.JSONDefault.Code, response.JSONDefault.Message)
	}
